...
class PanNukeDataModule(LightningDataModule):
    ...
    def setup(self, stage=None):
        if stage == 'fit' or stage is None:
            img_train = np.load(
                join(self.data_dir, 'images', self.train_fold, 'images.npy'),
                mmap_mode='r')
            img_valid = np.load(
                join(self.data_dir, 'images', self.valid_fold, 'images.npy'),
                mmap_mode='r')
            mask_train = np.load(
                join(self.data_dir, 'masks', self.train_fold, 'masks.npy'),
                mmap_mode='r')
            mask_valid = np.load(
                join(self.data_dir, 'masks', self.valid_fold, 'masks.npy'),
                mmap_mode='r')
        ...
        if stage == 'test' or stage is None:
            img_test = np.load(
                join(self.data_dir, 'images', self.test_fold, 'images.npy'),
                mmap_mode='r')
            mask_test = np.load(
                join(self.data_dir, 'masks', self.test_fold, 'masks.npy'),
                mmap_mode='r')
